...
fn main() {
    ...
    app.connect_activate(|app| {
        ...
        check.connect_clicked(move |_| { let repo = target_clone.text().to_string(); if repo.split('/').count() != 2 { status_clone.set_text("Expected owner/repo."); return; } status_clone.set_text("Inspecting release…"); let status = status_clone.clone(); let score = score_clone.clone(); let assets = assets_clone.clone(); std::thread::spawn(move || { let result = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async move { reqwest::Client::new().get(format!("https://api.github.com/repos/{repo}/releases/latest")).header("User-Agent", "ReleaseGuardGtk/0.1").send().await?.json::<Release>().await }); gtk::glib::MainContext::default().spawn_local(async move { match result { Ok(release) => { let value = 100_i32 - if release.assets.is_empty() { 40 } else { 0 }; score.set_text(&format!("{} · {value}/100", release.tag_name)); for asset in release.assets { assets.append(&Label::new(Some(&format!("{} · {} bytes · {}", asset.name, asset.size, asset.browser_download_url)))); } status.set_text("Release loaded. Asset URLs are ready to open."); }, Err(_) => status.set_text("Could not read this public release.") } }); }); });
        window.present();
    }); app.run();
}
